/**
 * One-shot ETL: legacy MySQL portal (ambulanc_web) → Rampart Postgres.
 * Spec §8. Idempotent — re-runnable against a partially migrated database.
 *
 * Usage:
 *   LEGACY_MYSQL_URL=mysql://user:pass@host:3306/ambulanc_web npx tsx scripts/migrate-legacy.ts
 *
 * Requires the Rampart database to be migrated and seeded first
 * (credential types, cert types, event kinds, roles).
 */
...
async function main() {
  ...
  const db = await mysql.createConnection({ uri: legacyUrl, dateStrings: true });
  ...
  console.log(`Members: ${realMembers.length} (+${placeholders.size} placeholders)`);
  ...
  console.log(`Default crew template: ${defaults.length} weekdays`);
  ...
  console.log(`Games: ${games.length} (${gamesCrews.length} signups)`);
  ...
  console.log(`Events: ${events.length} (${attendees.length} attendees)`);
  ...
  console.log(`Fuel log: ${fuel.length}`);
  ...
  console.log(`Radios: ${radios.length} (${radioLog.length} log rows)`);
