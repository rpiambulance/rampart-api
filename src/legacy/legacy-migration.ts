...
const CREW_POSITIONS: Array<
  [string, 'CC' | 'DRIVER' | 'ATTENDANT' | 'OBSERVER' | 'DUTY_SUP']
> = [
  ['cc', 'CC'],
  ['driver', 'DRIVER'],
  ['attendant', 'ATTENDANT'],
...
export async function runLegacyMigration(
  ...
  const certTypes = new Map(
    (await prisma.certificationType.findMany()).map((t) => [
      t.abbreviation,
      t.id,
    ]),
  );
  ...
  const placeholders = new Map<
    number,
    string
  > // negative pseudo-members
  (
    legacyMembers
      .filter((m) => m.id <= 0)
      .map((m) => [Number(m.id), m.last_name || 'CLOSED']),
  );
  ...
  for (const m of realMembers) {
    ...
    if (email === syntheticEmail) {
      record(
        `  ${label}: no email in legacy data; stored as ${syntheticEmail}`,
      );
    }
    ...
    try {
        ...
        if (!existing) {
          await prisma.memberRole.create({
            data: {
              memberId: member.id,
              roleId: role.id,
              // The agency's calendar day, not the UTC one: for the last four
              // hours of every evening they are different, and a role dated
              // tomorrow confers nothing until tomorrow.
              startDate: nyToday(),
            },
          });
        }
      ...
      if (cleanDate(m.cpr_exp)) {
        certRows.push({
          abbr: 'CPR',
          identifier: m.cpr_assoc || undefined,
          expiresAt: cleanDate(m.cpr_exp),
        });
      }
      if (cleanDate(m.emt_exp) || m.emt_num > 0) {
        certRows.push({
          abbr: 'EMT',
          identifier:
            [m.emt_level, m.emt_num > 0 ? `#${m.emt_num}` : null]
              .filter(Boolean)
              .join(' ') || undefined,
      ...
      if (cleanDate(m.dl_exp)) {
        certRows.push({
          abbr: 'DL',
          identifier: m.dl_state || undefined,
          expiresAt: cleanDate(m.dl_exp),
        });
      }
  ...
  const slotValue = (legacy: number) => {
    ...
    if (legacy < 0 || placeholders.has(legacy)) {
      return {
        memberId: null,
        placeholder: placeholders.get(legacy) ?? 'CLOSED',
      };
    }
    return {
      memberId: memberIdByLegacy.get(legacy) ?? null,
      placeholder: null,
    };
  };
  ...
  for (const d of defaults) {
    await guardRecord(
      ctx,
      'DefaultCrewTemplate',
      `legacy default crew weekday ${d.day}`,
      async () => {
        for (const [legacyCol, position] of CREW_POSITIONS) {
  ...
  for (const g of games) {
    ...
    await guardRecord(
      ctx,
      'Event',
      `legacy game ${g.id} (${g.description || 'Game'})`,
      async () => {
        const signups = gamesCrews.filter(
          (s) => Number(s.gameid) === Number(g.id),
        );
        const positionCounts = new Map<string, number>();
        for (const s of signups) {
          positionCounts.set(
            s.position,
            (positionCounts.get(s.position) ?? 0) + 1,
          );
        }
        const standard: Array<[string, number, string | null]> = [
          ...
          ...(g.ees === 1
            ? ([
                ['ees', Math.max(2, positionCounts.get('ees') ?? 0), 'EES'],
        ...
        const startsAt = nyToUtc(g.date, g.start, (bad) =>
          record(
            `  legacy game ${g.id}: unusable start time "${bad}"; used midnight`,
          ),
        );
        if (!startsAt) {
          record(
            `  ! legacy game ${g.id}: unusable date/time (${g.date} ${g.start}); skipped`,
          );
          return;
        }
        let endsAt = nyToUtc(g.date, g.end, (bad) =>
          record(
            `  legacy game ${g.id}: unusable end time "${bad}"; used midnight`,
          ),
        );
        if (!endsAt) {
          record(
            `  legacy game ${g.id}: unusable end time (${g.end}); ended at start`,
          );
          endsAt = startsAt;
        ...
        const event = await prisma.event.upsert({
          ...
          create: {
            ...
            positions: {
              create: standard.map(
                ([position, count, requiredCredentialKey]) => ({
                  position,
                  count,
  ...
  for (const e of events) {
    ...
    await guardRecord(
      ctx,
      'Event',
      `legacy event ${e.id} (${e.description || 'Event'})`,
      async () => {
        const cap = Number(e.limit);
        const startsAt = nyToUtc(e.date, e.start, (bad) =>
          record(
            `  legacy event ${e.id}: unusable start time "${bad}"; used midnight`,
          ),
        );
        if (!startsAt) {
          record(
            `  ! legacy event ${e.id}: unusable date/time (${e.date} ${e.start}); skipped`,
          );
          return;
        }
        let endsAt = nyToUtc(e.date, e.end, (bad) =>
          record(
            `  legacy event ${e.id}: unusable end time "${bad}"; used midnight`,
          ),
        );
        if (!endsAt) {
          record(
            `  legacy event ${e.id}: unusable end time (${e.end}); ended at start`,
          );
          endsAt = startsAt;
        } else if (endsAt < startsAt) {
          endsAt = new Date(endsAt.getTime() + DAY_MS);
        ...
        for (const a of attendees.filter(
          (a) => Number(a.eventid) === Number(e.id),
        )) {
  ...
  for (const f of fuel) {
    ...
    await guardRecord(
      ctx,
      'FuelLogEntry',
      `legacy fuel log ${f.date} ${f.vehicle}`,
      async () => {
        const loggedAt = nyToUtc(f.date, f.time, (bad) =>
          record(
            `  legacy fuel log ${f.date} ${f.vehicle}: unusable time "${bad}"; used midnight`,
          ),
        );
        if (!loggedAt) {
          record(
            `  ! legacy fuel log ${f.date} ${f.vehicle}: unusable date/time; skipped`,
          );
          return;
        }
  ...
  for (const r of radios) {
    await guardRecord(ctx, 'Radio', `legacy radio ${r.id}`, async () => {
      ...
      for (const log of radioLog.filter(
        (l) => Number(l.radio) === Number(r.id),
      )) {
        ...
        if (!open) {
          await prisma.radioAssignment.create({
            data: {
              radioId: radio.id,
              memberId: holder,
              issuedAt: cleanDate(r.issuedate) ?? nyToday(),
            },
          });
        }
      }
